		...
		<div class="col-md-7">
			<div class="box box-warning">
				<div class="box-body box-profile">
					...
					<button type="submit" class='btn btn-sm btn-primary pull-left' onClick=add_function('<?php echo $mode;?>'); id="submit" name="submit" value="submit">TAMBAH MEDIA</button>
					<br><br>
...
$(document).ready(function(){
	$('#my-table').DataTable({responsive: false,
		...
		"order": [[ 0, "desc" ]],
        "ajax" : {
            url: "<?php echo site_url("ujian_upload/upload_ajax"); ?>",
            type: "post",
            error: function() {
                $(".my-table-error").html("");
...
{
    var base_url = "ujian_upload/form?m="+m;
    window.location.href = "<?php echo site_url(); ?>"+base_url;
}

function edit_function(m,id) 
{
	var base_url = "ujian_upload/form?m="+m+"&id="+id;
	window.location.href = "<?php echo site_url(); ?>"+base_url;
}
...
function del_function(m,id,lampiran) 
{
    ...
    {
        var base_url = "ujian_upload/delete_upload?m="+m+"&id="+id+"&fl="+lampiran;
        window.location.href = "<?php echo site_url(); ?>"+base_url;
    }else{}
}

function delklik_function(m,id,lampiran) 
{
    if (confirm('Apakah Anda Yakin?'))
    {
        var base_url = "ujian_upload/delete_upload?m="+m+"&id="+id+"&fl="+lampiran;
